...
Every enrichment pass re-verifies the 25 stalest stored links (oldest `verifiedAt` first), so the ~97-link catalogue rotates about every four syncs. Sxyprn links are checked by fetching the watch page; Eporner links by the API's `video/id` lookup. Each check is bounded by a 15-second timeout, so a stalled connection cannot hold the shared fetch pool open. Only definitive non-existence counts as a strike - an sxyprn 404/410 or an eporner lookup that finds no record. Timeouts, 403 anti-bot walls, 5xx responses and network errors are inconclusive: the link is left untouched and retried next cycle. After two consecutive definitive failures the link moves out of `videoUrls` into `deadVideoUrls` (each entry keeps `verifiedAt` plus `deadAt` and `deadReason`), so `videoUrls` stays live-links-only and the UI needs no change. A successful verify resets the strike count and refreshes `verifiedAt`. When a scene's last live link dies, `videoCheckedAt` is cleared so the scene re-enters normal resolution on the next pass; the lookup results are filtered against `deadVideoUrls`, so a re-resolve cannot put a proven-dead URL back into `videoUrls`. Re-verify runs through the same bounded fetch pool as the rest of the sync - no new fan-out.
