...
The dashboard's **Refresh data** action updates studio records first, then starts Sxyprn matching in the server process. Matching saves the batch after it finishes and never removes a studio record when Sxyprn is unavailable. A linked thumbnail opens Liszt's watch page, which asks the server to resolve the stream before playback. Sxyprn stream URLs are bound to the requesting IP, so the server caches the short-lived URL and proxies range requests to the HTML video element. The page retains a direct Sxyprn post link if playback fails.
