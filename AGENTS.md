...
10. **State your authorship.** Every agent-authored PR names the agent and the model in its description (e.g. `Agent: OpenHands. Model: openhands/deepseek-v4.1-flash`). Subsequent edits keep it current: each new commit records its author in the commit message (e.g. a `Agent: OpenHands (deepseek-v4.1-flash)` trailer), and the description is updated if the agent or model changes mid-PR.
