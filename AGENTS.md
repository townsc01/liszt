...
2. **Claim and report the task.** Assign the Linear issue to the working agent or person, set it In Progress, and comment with the agent/model and intended branch. If the agent cannot update Linear, report the same information to the invoking coordinator and do not pretend the status changed. **Comment discipline:** the Linear issue is the shared record - comment when you start (plan in two sentences), on any load-bearing decision or blocker, and at handoff (PR link, what changed, what you verified). If you cannot write to Linear, send each of those to the invoking coordinator so it posts them for you.
